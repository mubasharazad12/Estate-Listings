...
const Navbar = () => {
  const [proflieName, setproflieName] = useState('Mubashar');
  const [darkCheckBox, setdarkCheckBox] = useState<boolean>(false);
  function test (e: { target: { checked: boolean | ((prevState: boolean) => boolean); }; }){ 

    setdarkCheckBox(e.target.checked);
  ...
  return ( 
    <>
      <header className="flex justify-between w-full flex-wrap p-8 shadow-1xl  shadow-shadow sticky top-0  z-10  bg-secondary h-28 items-center  bg-gradient-to-l from-gradient ">
          ...
          {/* <input type="checkbox" checked={darkCheckBox} onChange={e => setdarkCheckBox(e.target.checked)} /> */}
            <li className='flex justify-center items-center gap-2 '>
              <span className='flex'>  
                { darkCheckBox && <box-icon name='moon' type="solid"></box-icon> } { !darkCheckBox && <box-icon name='moon' ></box-icon>  } 
              </span>
              <label className="relative inline-flex items-center cursor-pointer">
                <input type="checkbox"  checked={darkCheckBox} onChange={test} className="sr-only peer" />
